...
__global__ void rasterize_bwd_kernel(
    ...
    for (int32_t b = 0; b < num_batches; ++b) {
        ...
        for (int32_t t = 0; t < batch_size; ++t) {
            ...
            float gauss_2d = 2.0f * (dx*dx + dy*dy);
            ...
            if (opac * vis <= 0.999f) {
                if (gauss_3d <= gauss_2d) {
                    ...
                    float v_s_x = v_G * (-vis) * u;
                    float v_s_y = v_G * (-vis) * v;
                    ...
                    // h_u = px*wM - uM  →  d/d(uM)=-1,  d/d(wM)=px
                    v_uM0 = -v_h_u.x;   v_uM1 = -v_h_u.y;   v_uM2 = -v_h_u.z;
                    v_vM0 = -v_h_v.x;   v_vM1 = -v_h_v.y;   v_vM2 = -v_h_v.z;
                    v_wM0 = px * v_h_u.x + py_f * v_h_v.x;
                    v_wM1 = px * v_h_u.y + py_f * v_h_v.y;
                    v_wM2 = px * v_h_u.z + py_f * v_h_v.z;

                } else {
                    // Case 2: 2D screen falloff — gradient goes to means2d
                    float v_G_dx = -vis * 2.0f * dx;
                    float v_G_dy = -vis * 2.0f * dy;
                    v_xy0 = v_G * v_G_dx;
                    v_xy1 = v_G * v_G_dy;
...
void launch_rasterize_bwd(
    ...
    size_t smem = 256u * (sizeof(int32_t) + sizeof(bwd_f3)*4u + sizeof(float)*3u);

    rasterize_bwd_kernel<<<grid, block, smem>>>(
        d_means2d, d_ray_transforms, d_opacities, d_colors,
        d_tile_offsets, d_flatten_ids, n_isects,
        d_render_alphas, d_last_ids, d_v_render_colors,
        image_width, image_height, tile_width, tile_height,
        d_v_ray_transforms, d_v_opacities, d_v_colors,
